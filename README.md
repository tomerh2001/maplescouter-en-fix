...
- **Sync icon** with a tooltip: not uploaded, synced, edited since the last upload, cloud copy newer, conflict, or unavailable. Click it to upload, load the cloud copy, or replace it. When the two copies differ you get one question with two or three buttons, and a **Show differences** link lists the fields that changed. It checks every five minutes while the Character page is visible, with less frequent checks after failures. Focus checks run at most once a minute. Uploads are always explicit (no auto-upload).
