...
- **Sync icon** with a tooltip: not uploaded, synced, edited since the last upload, cloud copy newer, conflict, or unavailable. Click it to upload, load the cloud copy, or replace it. When the two copies differ you get one question with two or three buttons, and a **Show differences** link lists the fields that changed. It checks when the Character page opens, when you return to the tab, and every five minutes while visible. Opening the picker also checks the listed characters. Repeat checks share a one-minute cooldown, and failed checks back off. A pending check shows a neutral icon instead of a cached green one. Uploads are always explicit (no auto-upload).
...
- **Keep the character's region.** Loading a saved character also selects its GMS, KMS, TMS, JMS, or MSEA region. Local saves and calculations still work when cloud requests are blocked.
- **Save dates.** Matching `Local:` and `Cloud:` labels use seconds, minutes, hours, days, months, or years. Dates update while the picker is open, without extra network requests.
